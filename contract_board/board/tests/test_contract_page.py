...
class ContractPageTest(FunctionalTest):
    ...
    def validate_contract(self,
        ...
        self.assertEqual(title.text, contract_dict['contract_title'])
        name = contract_listing.find_element(By.CLASS_NAME, 'contract-agency-name')
        self.assertEqual(name.text, contract_dict['agency_name'])
        contact = contract_listing.find_element(By.CLASS_NAME, 'contract-contact-information')
        self.assertEqual(contact.text, contract_dict['contact_information'])
        end_date = contract_listing.find_element(By.CLASS_NAME, 'contract-bidding-end-date')
        self.assertEqual(end_date.text, contract_dict['bidding_end_date'])
        lowest_bid = contract_listing.find_element(By.CLASS_NAME, 'contract-lowest-bid')
        if bid is None:
            self.assertEqual(lowest_bid.text, 'None')
        else:
            self.assertEqual(float(lowest_bid.text[1:]), float(bid))
        description = contract_listing.find_element(By.CLASS_NAME, 'contract-job-description')
        self.assertEqual(description.text, contract_dict['job_description'])

    def validate_bid_listing(self,
                             bid_listing: WebElement,
                             bid: dict[str, str]) -> None:
        """
        ...
        self.assertEqual(title.text, bid['contractor_name'])
        amount = bid_listing.find_element(By.CLASS_NAME, 'bid-amount')
        self.assertEqual(float(amount.text[1:]), + float(bid['amount']))
        contact = bid_listing.find_element(By.CLASS_NAME, 'bid-contact-information')
        self.assertEqual(contact.text, bid['contact_information'])
        list_date = bid_listing.find_element(By.CLASS_NAME, 'bid-date-placed')
        self.assertEqual(list_date.text, date.today().strftime('%m/%d/%y'))

    def test_contract_page(self) -> None:
        ...
        self.assertEqual(contractor_name.text, bid2['contractor_name'])
    ...
    def test_cannot_submit_invalid_bid_form(self) -> None:
        pass